...
- 中国大陆 App Store 上架：APP 备案号 `蜀ICP备2026054850号-2A` 已填进 App Store Connect（分发 › App 信息 › App Store 法规和许可 › 中国大陆 ICP 备案号），价格与销售范围里勾上中国大陆（147 → 148 个国家或地区，24 小时内生效），官网部署在备案域名 <https://ls.mixduo.cn/>，页脚印网站备案号 `蜀ICP备2026054850号`（与 ASC 里的 APP 备案号不是同一个）。简体中文的隐私政策 URL 要等 macOS 0.4.1 出审核（App 信息被审核锁定），技术支持 URL 是版本级字段随下个版本改；过程与剩余步骤记在 `docs/store-listing.md` §二。
...
- 提示条点任意空白处即可全部消掉，不必等它自己超时或逐条点。提示条容器本身不再拦截点击，只有卡片接收，周围区域归下面的界面。

### Fixed
- 手机上的提示条不再压住底部标签栏。移动端那条避让规则写在样式表靠前的媒体查询里，被后面同优先级的基础规则整个盖掉，一直没生效；移到基础规则之后，并且改用实测的标签栏高度（`Layout.tsx` 用 ResizeObserver 写进 `--tabbar-height`）而不是写死的常量——德语等语言的标签文字换行时标签栏会变高，常量必然失准。

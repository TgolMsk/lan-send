...
## 12. 里程碑 2 决策记录（2026-09-07，用户确认）

| # | 问题 | 决定 |
|---|---|---|
| 1 | 文件夹传输 | 用协议 v2 的 `fileName` 相对路径（官方 1.9 起支持），不做 zip 回退 |
| 2 | 接收目录按设备 / 日期 / 类型分子目录 | 放里程碑 2 |
| 3 | 断点续传 | 只在双方都声明 `resume` 时启用；断点状态 24 小时有效，会话结束即失效 |
| 4 | 传输历史 | 默认保留 200 条，可在设置中修改 |

## 13. 里程碑 3 决策记录（2026-09-07，用户确认）

| # | 问题 | 决定 |
|---|---|---|
| 1 | 配对交互 | 方案 A：两端各自显示由双方指纹派生的同一个 6 位校验码，两边都确认一致即配对 |
| 2 | 剪贴板同步方向 | 双向；配对即授权 |
| 3 | 敏感内容 | 按启发式判定（无空格、长度 20–128、字符熵高）只同步不入历史 |
| 4 | iOS | 不做剪贴板同步 |
| 5 | Windows 实测 | 用户有 Windows 机器，可用 CI 产物实测 |
